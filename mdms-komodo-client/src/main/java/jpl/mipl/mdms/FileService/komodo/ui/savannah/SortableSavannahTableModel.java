...
/**
 * <b>Purpose: </b> TableSorter is a decorator for TableModels; adding sorting
 * functionality to a supplied TableModel. TableSorter does not store or copy
 * the data in its TableModel; instead it maintains a map from the row indexes
 * of the view to the row indexes of the model. As requests are made of the
 * sorter (like getValueAt(row, col)) they are passed to the underlying model
 * after the row numbers have been translated via the internal mapping array.
 * This way, the TableSorter appears to hold another copy of the table with the
 * rows in a different order. <p/>TableSorter registers itself as a listener to
 * the underlying model, just as the JTable itself would. Events recieved from
 * the model are examined, sometimes manipulated (typically widened), and then
...
public class SortableSavannahTableModel extends AbstractTableModel implements
        SavannahTableModel
{
    protected SavannahTableModel   tableModel;
    ...
    public SortableSavannahTableModel()
    {
        this(new DefaultSavannahTableModel(), null);
    }

    //---------------------------------------------------------------------

    public SortableSavannahTableModel(SavannahTableModel tableModel)
    {
    ...
    public void setTableModel(SavannahTableModel tableModel)
    {
    ...
    public int modelIndex(int viewIndex)
    {
        if (viewIndex < 0)
            return -1;
        Row[] conversionArray = getViewToModel();
        if (viewIndex >= conversionArray.length)
            return -1;
        return conversionArray[viewIndex].modelIndex;
    }
    ...
    {
        //build modelToView array if necessary
        if (modelToView == null)
        {
    ...
    public int viewIndex(int modelIndex)
    {
        if (modelIndex < 0)
            return -1;
        int[] conversionArray = getModelToView();
        if (modelIndex >= conversionArray.length)
            return -1;

        return conversionArray[modelIndex];
    }
    ...
    public void addAll(List elementList)
    {
        tableModel.addAll(elementList);
    }
    ...
    {
        tableModel.addAll(elementArray);
    }
    ...
    {
        return tableModel.elementAt(modelIndex(index));
    }

    //---------------------------------------------------------------------

    /**
     * Returns the index of element argument if found in internal element list,
     * or -1 if not found.
    ...
    {
        return viewIndex(tableModel.indexOf(element));
    }
    ...
    private class Row implements Comparable
    {
        ...
        public int compareTo(Object o)
        {
            ...
            for (Iterator it = sortingColumns.iterator(); it.hasNext();)
            {
                ...
                SavannahTableElement element1, element2;
                element1 = (SavannahTableElement) tableModel.elementAt(row1);
                element2 = (SavannahTableElement) tableModel.elementAt(row2);
                ...
                else
                {
                    if (element1.getType() == element2.getType())
                        ...
                        if (element1.getType() == SavannahTableElement.
                                                                  TYPE_FOLDER)
                            comparison = directive.direction == DESCENDING ?
                                                                     1 : -1;
                        else
                            comparison = directive.direction == DESCENDING ?
                                    -1 : 1;
        ...
        public void tableChanged(TableModelEvent e)
        {
                    ...
                    && modelToView != null)
            {
                ...
                fireTableChanged(new TableModelEvent(
                        SortableSavannahTableModel.this, viewIndex, viewIndex,
                        column, e.getType()));
                return;
            }
